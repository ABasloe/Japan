    ...
    (function(){
      ...
      function applyFilters(){
        document.querySelectorAll('.day-section').forEach(function(section){
          // A text search is deliberately global.  A city chip still narrows
          // the itinerary when there is no search phrase, and resumes doing
          // so as soon as the phrase is cleared.
          var cityMatch=!!query||selected==='all'||section.dataset.city===selected, visible=0;
      ...
      window.addEventListener('jp-shared-changed',applyFilters);
      new MutationObserver(applyFilters).observe(document.getElementById('agenda-list'),{childList:true});
...
def build_collaboration_app(routes):
    ...
    (function(){
      ...
      function options(){var o='';for(var d=1;d<=DAY_COUNT;d++)o+='<option value="'+d+'">Day '+d+' — '+esc(DAY_LABELS[d]||'')+'</option>';return o}document.getElementById('stop-day').innerHTML=options();
      ...
      function selectedLocation(lat,lon,label){var name=document.getElementById('stop-name'),address=document.getElementById('stop-address'),maps=document.getElementById('stop-maps');document.getElementById('stop-lat').value=Number(lat).toFixed(6);document.getElementById('stop-lon').value=Number(lon).toFixed(6);if(!name.value.trim())name.value=String(label).split(',')[0];if(!address.value.trim())address.value=label;if(!maps.value.trim())maps.value='https://www.google.com/maps?q='+encodeURIComponent(lat+','+lon);document.getElementById('place-status').textContent='Location loaded: '+label;document.getElementById('place-results').innerHTML=''}
